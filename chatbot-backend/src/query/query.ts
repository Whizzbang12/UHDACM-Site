...
import { VectorDBBaseMetadata, VectorDBPageMetadataAction } from "@shared/types/vectorDB/vectorDBTypes";
...
function checkQueryResponse(obj: unknown): asserts obj is QueryResponse {
  ...
  for (const action of relevant_actions) {
    if (typeof action.label !== 'string' || typeof action.href !== 'string') {
      throw new Error('Invalid relevant_actions: each action must have a string label and href');
    }
  }
};
...
export const processQuery = async (query: string) => {
  ...
  while (true) {
    try {
      ...
      currentResponse = await handleQuestion(`${currentResponse}\nFix Error: ${(e as Error).message}`, 'gemma-3-1b-it');
    }
  }
  return {
